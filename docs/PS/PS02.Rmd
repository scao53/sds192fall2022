...
We're only going to only focus on data for 2007 and save this in a new data frame `gapminder_2007`. We use data wrangling using code from the `dplyr` package. Don't worry if this doesn't make sense for now, we'll cover data wrangling later in this course. For now, run the following code:
...
**Question**: What country had the highest GDP per capita in 2007? Use the sorting arrows next to variable names in RStudio's `View()` function.
